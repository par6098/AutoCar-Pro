...
export function DashboardPage({ onNavigate }: { onNavigate: (page: PageId)=>void }) {
  ...
  return <>
    ...
    <div className="grid grid-2" style={{marginTop:14}}>
      ...
      <div className="card"><div className="card-title">Today's jobs</div>{data.todaysJobs.map(job => <div key={job.id} style={{display:'flex', justifyContent:'space-between', padding:'9px 0', borderBottom:'1px solid var(--border)'}}><div><b>{job.vehicle}</b><div className="muted">{job.serviceName} · {job.schedule}</div></div><Badge tone={job.status === 'Completed' ? 'green' : job.status === 'In Progress' ? 'blue' : 'amber'}>{job.status}</Badge></div>)}</div>
    </div>
  </>;
}
